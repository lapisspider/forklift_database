...
from .models import Forklift, Kit, User
...
def _needs_admin(method: str, path: str) -> bool:
    """Editing/managing requires admin; browsing + AI lookups don't."""
...
async def auth_guard(request: Request, call_next):
    ...
    if _needs_admin(request.method, path) and user.get("role") != "admin":
        return JSONResponse({"detail": "Admins only — you have view-only access."},
                            status_code=403)
    return await call_next(request)
...
async def create_kit(request: Request, db: Session = Depends(get_db)):
    ...
    kit = Kit(
        ...
        forklifts=_forklifts_from_ids(db, form.getlist("forklift_ids")),
    )
    db.add(kit)
    db.commit()
...
async def edit_kit(kid: int, request: Request, db: Session = Depends(get_db)):
    ...
    kit.forklifts = _forklifts_from_ids(db, form.getlist("forklift_ids"))
    db.commit()
    return RedirectResponse(url="/#kits", status_code=303)
